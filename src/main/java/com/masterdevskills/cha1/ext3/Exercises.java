...
public class Exercises {
	...
	public static List<Person> sortByLastNameOrderDescending(List<Person> people) {
		Collections.sort(people, Comparator.comparing((Person o) -> o.getLastName()).reversed());
		return people;
 	}
	...
	public static List<Person> sortByFirstNameAndThenLastNameAndThenAge(List<Person> people) {
		Comparator<Person> firstName = Comparator.comparing(Person :: getFirstName);
		Comparator<Person> lastName = Comparator.comparing(Person :: getLastName);
		Comparator<Person> age = Comparator.comparing(Person :: getAge);

		Comparator<Person> comparator = firstName.thenComparing(lastName).thenComparing(age);
